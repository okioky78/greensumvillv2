...
export const ReceiptDriveUploadPage = () => {
  ...
  const alertAuthRequired = (message: string) => {
    ...
    extractPaymentDate(imageSelection.file, {
      ...
      onError: (err) => {
        ...
        if (isAuthRequiredError(err)) {
          alertAuthRequired(getApiErrorMessage(err, "결제일 추출 중 오류가 발생했습니다."));
          return;
        }
    ...
    uploadToDrive(
      ...
      },
      {
        ...
        onError: (err) => {
          if (!isCurrentFlow(flowVersion)) return;
          if (isAuthRequiredError(err)) {
            alertAuthRequired(getApiErrorMessage(err, "구글 드라이브 저장 중 오류가 발생했습니다."));
            return;
          }
  ...
  useEffect(function clearSessionWhenBranchesRequireAuth() {
    ...
    if (driveBranches.errorMessage) {
      window.alert(driveBranches.errorMessage);
    }

    googleConnection.clearAuthenticatedState();
    redirectToGoogleLogin();
  }, [
    driveBranches.errorMessage,
    driveBranches.isAuthRequired,
    googleConnection.clearAuthenticatedState,
  ]);
            ...
            {hasSelectedFile && (
              <section className="space-y-6">
                <AnimatePresence mode="wait">
                  <SaveDetailsCard
                    file={imageSelection.file}
                    branchError={driveBranches.errorMessage}
