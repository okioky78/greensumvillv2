...
import { HttpError, getPublicErrorCode, getPublicErrorMessage } from "../shared/http.ts";
...
const getErrorMessage = (error: unknown) =>
  error instanceof HttpError
    ? getPublicErrorMessage(getPublicErrorCode(error))
    : getPublicErrorMessage("AUTH_FAILED");
...
export const handleGoogleAuthCallback = async (
  ...
  try {
    ...
    if (error instanceof HttpError && error.code === "OAUTH_DENIED") {
      return {
